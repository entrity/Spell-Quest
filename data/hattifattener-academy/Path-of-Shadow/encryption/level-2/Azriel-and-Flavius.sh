...
tutorial () {
	...
	* $(spell gpg)${AZRIEL} makes use of two different key/passwords; one key is used the encrypt the message and the other key is used to decrypt it.
	...
	${FLAVIUS}But assymetric-key cryptography provides a solution to both of these problems. Under this scheme, I create $(alt two)${FLAVIUS} keys: a private one, which I keep secret, and a public one, which I can share with everyone.
	...
	* This will write the decrypted content to your terminal, so be sure to use a redirect if you want to write it to a file. (This is very important if the file is large.)
	...
	${FLAVIUS}Oh, but that was a lot of materials. Be sure to use $(spell lessons)${FLAVIUS} to review all of these invocations. (You might want to pipe its output to $(spell grep)${FLAVIUS} and use $(spell grep)${FLAVIUS} to just search for lessons that include the word $(alt gpg)${FLAVIUS}.)
