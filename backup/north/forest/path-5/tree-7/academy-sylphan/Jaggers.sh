...
When school is in session, we teach only first-level spells in this academy, but a book full of first-level spells is enough to take you far in this world. And it's a good deal more than most people ever know.
...
if ! [[ -e "$HOME/bag" ]]; then
	...
	Then go see what this academy has to offer. The elderly man mentioned a $(alt Transmutaiton professor) and someone else, but I don't recall the name he said.
	HEREDOC
fi
